...
def write_adaptive_section_report(
    ...
    *,
    ...
    warnings: list[str] = []
    if mastering_cfg.get("mode") in {"global_master_slices", "composition_master"}:
        if section_postprocess_ids:
            warnings.append(
                "composition-master slicing active: section-local legacy postprocess blocks are ignored for exported full mixes "
...
def write_adaptive_composition_mastering_report(
    ...
    *,
    ...
    if mastering_cfg.get("mode") not in {"global_master_slices", "composition_master"} and len(section_order) > 1:
        warnings.append(
            "adaptive full-section assets are not configured for composition-master slicing; "
            "verify this is intentional before shipping a horizontally crossfaded cue"
        )
    if section_post and mastering_cfg.get("mode") in {"global_master_slices", "composition_master"}:
        warnings.append(
            "section-local legacy postprocess blocks exist but composition-master slicing ignores them for full-section export: "
            + ", ".join(section_post)
        )
    if good:
