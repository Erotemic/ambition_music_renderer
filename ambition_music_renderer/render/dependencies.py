...
RENDER_DEPENDENCY_SCHEMA = "ambition.render_dependencies.v2"
...
def _audio_settings_payload(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "processing": copy.deepcopy(spec.get("processing", {}) or {}),
        "master_processing_plan": processing_plan_for_master(spec).as_dict(),
        "mastering_policy": mastering_policy(spec).as_dict(),
...
def processing_dependency_payload(
    ...
    *,
    ...
    for path, raw_spec in collect_effect_specs(dict(spec)):
        ...
        raw_processor = effect_spec.get("processor")
        if raw_processor is not None:
            ...
            nested_pedalboard_effect = False
        else:
            kind = str(
            ...
            from ..processing.catalog import is_pedalboard_effect_name

            nested_pedalboard_effect = is_pedalboard_effect_name(kind)
        row: dict[str, Any] = {}
        if kind in {"pedalboard", "pb"} or nested_pedalboard_effect:
