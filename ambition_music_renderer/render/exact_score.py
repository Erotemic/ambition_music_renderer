...
class ExactTempoMap:
    ...
    def time_to_tick(self, seconds: float, *, hint_max_tick: int | None = None) -> int:
        """Invert :meth:`tick_to_time` to the nearest integer score tick.

        Exact-score compilation stores PrettyMIDI events in seconds because
        synthesis backends consume wall-clock time. DAW interchange needs the
        authored tick grid back. The map is monotone; holds introduce time gaps
        but events generated by MusicIR sit on representable tick times, so an
        integer binary search recovers their source coordinate exactly.
        """

        target = max(0.0, float(seconds))
        lo = 0
        hi = max(1, int(hint_max_tick or self.ppq * 4))
        while self.tick_to_time(hi) < target:
            hi *= 2
            if hi > 2_000_000_000:
                raise ValueError(f"could not invert score time {seconds!r}")
        while lo < hi:
            mid = (lo + hi) // 2
            if self.tick_to_time(mid) < target:
                lo = mid + 1
            else:
                hi = mid
        candidates = [max(0, lo - 1), lo, lo + 1]
        return min(candidates, key=lambda tick: abs(self.tick_to_time(tick) - target))
...
def compile_exact_score(
    ...
    *,
    ...
    canonical_schema: str = EXACT_SCHEMA,
    normalized_spec_override: dict[str, Any] | None = None,
    ...
    for part in spec.get("parts", []) or []:
        ...
        # Part-level exact controller events.  V2 historically supported CC
        # points here; pitch-bend points are an additive extension used by V3
        # clip automation and DAW interchange.
        for control_index, control in enumerate(part.get("controls", []) or []):
            ...
            source_ref = copy.deepcopy(control.get("_source_ref"))
            event_id = control.get("_event_id")
            if "pitch_bend" in control or "bend" in control:
                pitch = int(clamp(control.get("pitch_bend", control.get("bend")), -8192, 8191))
                instruments[target].pitch_bends.append(
                    pretty_midi.PitchBend(pitch=pitch, time=at)
                )
                event_row = {
                ...
                if number_raw is None:
                    raise ValueError(
                        f"part {part_id!r} control needs cc/controller or pitch_bend: {control!r}"
                    )
                ...
                value = int(clamp(control["value"], 0, 127))
                instruments[target].control_changes.append(
                    pretty_midi.ControlChange(number=number, value=value, time=at)
                )
                event_row = {
            ...
            event_row["ordinal"] = control_index
            if source_ref is not None:
                event_row["source_ref"] = source_ref
            if event_id is not None:
                event_row["event_id"] = str(event_id)
            controller_events.append(event_row)
        ...
        for voice in part.get("voices", []) or []:
            ...
            for row in _event_rows(voice, phrases, clock):
                ...
                pitches = _pitch_list(row)
                source_ref = copy.deepcopy(row.get("_source_ref"))
                event_id_base = row.get("_event_id_base")
                for pitch_index, pitch in enumerate(pitches):
                    ...
                    if source_ref is not None:
                        ...
                        ref["pitch_index"] = pitch_index
                        event_row["source_ref"] = ref
                    if event_id_base is not None:
                        event_row["event_id"] = f"{event_id_base}/p{pitch_index:02d}"
                    note_events.append(event_row)
    ...
    compiled = CompiledScore(
        ...
        normalized_spec=copy.deepcopy(normalized_spec_override if normalized_spec_override is not None else spec),
        pm=pm,
        groups=groups,
        sections=meta,
        ...
        authoring_graph=copy.deepcopy(authoring_graph),
    )
