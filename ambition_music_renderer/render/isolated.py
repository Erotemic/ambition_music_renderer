...
SECTION_FULL_MASTERING_MODES = (
    "section_postprocess",
    "global_master_slices",
...
def adaptive_section_mastering_config(spec: dict) -> dict[str, object]:
    """Compatibility view of the canonical mastering policy.

    Historical manifests used ``section_postprocess`` / ``global_master_slices``.
    Internally Phase 6 uses ``legacy_section_postprocess`` / ``composition_master``.
    Explicit historical spellings are preserved in this compatibility payload so
    old report/tests and regenerated assets remain explainable.
    """
    from ..processing.mastering import mastering_policy, COMPOSITION_MASTER
    ...
    if not isinstance(cfg, dict):
        cfg = {}
    if policy.authored_mode in {"global_master_slices", "section_postprocess"}:
        compat_mode = policy.authored_mode
    else:
        compat_mode = policy.mode
    return {
        "mode": compat_mode,
        "canonical_mode": policy.mode,
        "policy_source": policy.source,
        "ignore_section_postprocess_for_full_mix": bool(
            cfg.get("ignore_section_postprocess_for_full_mix", policy.mode == COMPOSITION_MASTER)
        ),
...
def _render_main(ns) -> int:
    ...
    # ---- Canonical composition bus + master ----
    # Legacy score fields are normalized into the same ProcessingPlan objects,
    # while v3 can author explicit processing chains. Section-local canonical
    # processing happens before the one composition master.
    with timings.phase("mix_master_preview"):
        from ..processing.mastering import (
            apply_section_bus_processing,
            master_composition,
            mastering_policy,
        )

        raw_full_native = np.zeros((target, 2), dtype="float32")
        for arr in stem_audio.values():
            raw_full_native += arr
        raw_full, section_bus_processing = apply_section_bus_processing(
            raw_full_native, sr, spec, meta, base_dir=spec_path.parent
        )
        master, master_processing_plan = master_composition(
            raw_full, sr, spec, base_dir=spec_path.parent
        )
        mastering_policy_obj = mastering_policy(spec)
        preview = (
    ...
    raw_full_native_stats = _audio_stats(raw_full_native, sr)
    ...
    from ..processing.metrics import audio_regression_metrics, section_boundary_metrics
    signal_flow_regression = {
        "post_stem_sum": audio_regression_metrics(raw_full_native, sr),
        "pre_master": audio_regression_metrics(raw_full, sr),
        "mastered_timeline": audio_regression_metrics(master, sr),
        "mastered_section_boundaries": section_boundary_metrics(master, sr, meta),
    }
    ...
    section_full_mix_details: dict[str, dict[str, object]] = {}
    if not ns.simple_mix:
        from ..processing.mastering import adaptive_full_mix

        for sec in meta:
            piece, section_detail = adaptive_full_mix(
                mastered_timeline=master,
                pre_master_timeline=raw_full,
                sample_rate=sr,
                spec=spec,
                section=sec,
                base_dir=spec_path.parent,
            )
            section_full_mix_details[str(sec["id"])] = section_detail
            if section_detail.get("section_postprocess_ignored"):
    ...
    # Composition diagnostics are measurements over the canonical CompiledScore,
    # so every render carries the same evidence an authoring agent sees through
    # `cue quality` without introducing another interpretation of MusicIR.
    from ..audit.composition_quality import composition_quality_report
    manifest["composition_quality"] = composition_quality_report(compiled)

    manifest["diagnostics"] = {
        "raw_full_native": raw_full_native_stats,
        "raw_full": raw_full_stats,
        "mastered_full": master_stats,
        "processing": {
            "master_plan": master_processing_plan,
            "section_bus_plans": section_bus_processing,
            "mastering_policy": mastering_policy_obj.as_dict(),
            "adaptive_full_mix_details": section_full_mix_details,
            "signal_flow_regression": signal_flow_regression,
        },
