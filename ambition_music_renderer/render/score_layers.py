...
def render_layer_arpeggio(
    ...
    velocity_pattern = [float(x) for x in layer.get("velocity_pattern", [1.0])]
    if not velocity_pattern:
        raise ValueError("arpeggio velocity_pattern must not be empty")
    octave_pattern = [int(x) for x in layer.get("octave_pattern", [0])]
    if not octave_pattern:
        raise ValueError("arpeggio octave_pattern must not be empty")
    ...
    for local in range(int(section["bars"])):
        ...
        for i in range(count):
            if ctx.rng.random() > density:
                continue
            base_pitch = (
                tones[pattern[i % len(pattern)] % len(tones)]
                + 12 * octave_pattern[i % len(octave_pattern)]
            )
            for inst in insts:
                ...
                v = (
                    ...
                    + float(inst_velocity_offsets.get(inst, 0.0))
                )
                note_dur = dur
                if clip_to_bar:
...
def render_layer_bassline(
    ...
    velocity_pattern = [float(x) for x in layer.get("velocity_pattern", [1.0])]
    bar_velocity_pattern = [float(x) for x in layer.get("bar_velocity_pattern", [1.0])]
    if not velocity_pattern or not bar_velocity_pattern:
        raise ValueError("bassline velocity patterns must not be empty")
    ...
    for local in range(int(section["bars"])):
        ...
        for item_index, item in enumerate(pattern):
            interval, beat, dur = int(item[0]), float(item[1]), float(item[2])
            note_velocity = (
                velocity
                * velocity_pattern[item_index % len(velocity_pattern)]
                * bar_velocity_pattern[local % len(bar_velocity_pattern)]
            )
            add_note(
                ctx,
                inst,
                ...
                beat,
                dur,
                note_velocity * float(section.get("intensity", 1.0)),
                articulation=articulation,
                **hk,
            )
...
def render_layer_drums(
    ...
    events = list(layer.get("events", []) or [])
    fill_events = list(layer.get("fill_events", []) or [])
    if not events and not fill_events:
        return
    bar_velocity_pattern = [float(x) for x in layer.get("bar_velocity_pattern", [1.0])]
    if not bar_velocity_pattern:
        raise ValueError("drums bar_velocity_pattern must not be empty")
    fill_every = int(layer.get("fill_every_bars", 0) or 0)
    if fill_every < 0:
        raise ValueError("drums fill_every_bars must be >= 0")
    fill_mode = str(layer.get("fill_mode", "overlay"))
    if fill_mode not in {"overlay", "replace"}:
        raise ValueError("drums fill_mode must be 'overlay' or 'replace'")

    def render_events(rows: list[dict[str, Any]], *, local: int, bar_scale: float) -> None:
        for ev in rows:
    ...
    for local in range(int(section["bars"])):
        bar_scale = bar_velocity_pattern[local % len(bar_velocity_pattern)]
        is_fill_bar = bool(fill_every and fill_events and (local + 1) % fill_every == 0)
        if not (is_fill_bar and fill_mode == "replace"):
            render_events(events, local=local, bar_scale=bar_scale)
        if is_fill_bar:
            render_events(fill_events, local=local, bar_scale=bar_scale)
...
def render_layer_guitar_lead(
    ...
    octave_shifts = tuple(int(x) for x in layer.get("fretboard_octave_shifts", (-12, 0, 12)))
        ...
        for start in starts:
            ...
            for i, p0 in enumerate(notes):
                ...
                for j, inst in enumerate(insts):
                    ...
                    choices = gp.positions_for_pitch(
                        p, tuning=tuning, max_fret=max_fret, octave_shifts=octave_shifts
                    )
...
LAYER_RENDERERS = {
    "pad_chords": render_layer_pad_chords,
    "arpeggio": render_layer_arpeggio,
    "ostinato": render_layer_ostinato,
    "bassline": render_layer_bassline,
    "motif": render_layer_motif,
    "chord_hits": render_layer_chord_hits,
    "drums": render_layer_drums,
    "texture": render_layer_texture,
    "pedal": render_layer_pedal,
    "root_hits": render_layer_root_hits,
    "guitar_shape_pick": render_layer_guitar_shape_pick,
    "guitar_strum": render_layer_guitar_strum,
    "guitar_chug": render_layer_guitar_chug,
    "sampled_chord": render_layer_sampled_chord,
    "guitar_lead": render_layer_guitar_lead,
    "notes": render_layer_notes,
    "automation": render_layer_automation,
}
...
def render_layer(
    ...
    kind = str(layer["kind"])
    renderer = LAYER_RENDERERS.get(kind)
    if renderer is None:
        raise KeyError(
            f"unknown layer kind {kind!r}; expected one of {sorted(LAYER_RENDERERS)}"
        )
    ctx.dynamics_scale = _dynamics_scale_fn(section, layer, ctx.beats_per_bar)
    try:
        renderer(ctx, section, layer)
        if kind != "automation":
            apply_automation(ctx, section, layer)
    finally:
