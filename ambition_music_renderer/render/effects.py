...
def post_process(
    ...
    *,
    ...
    """Compatibility facade for the canonical processing pipeline.

    Historical v1/v2/v3 ``postprocess`` mappings are normalized exactly once
    into a :class:`ProcessingPlan`, then executed by ``processing.pipeline``.
    New code should prefer an explicit canonical processing plan.
    """
    from ..processing.legacy import legacy_processing_plan
    from ..processing.pipeline import apply_processing_plan

    plan = legacy_processing_plan(settings, stage="postprocess_compat")
    return apply_processing_plan(audio, sample_rate, plan, base_dir=base_dir)
