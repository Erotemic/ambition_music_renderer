...
def collect_effect_specs(score: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    # Canonical Phase-5 processing chains are a second source syntax for the
    # same processor registry, not another backend implementation. Collect
    # their steps here so dependency/plugin validation sees VST3/LV2/command
    # resources regardless of whether a score used legacy effect_chain syntax.
    processing = score.get("processing") or {}
    def add_processing_block(block_path: str, block: Any) -> None:
        if isinstance(block, list):
            for idx, row in enumerate(block):
                add_spec(f"{block_path}[{idx}]", row)
            return
        if not isinstance(block, dict):
            return
        if "chain" in block:
            for idx, row in enumerate(_as_list(block.get("chain"))):
                add_spec(f"{block_path}.chain[{idx}]", row)
    if isinstance(processing, dict):
        add_processing_block("$.processing.stems", processing.get("stems"))
        add_processing_block("$.processing.master", processing.get("master"))
        for family in ("groups", "sections"):
            rows = processing.get(family) or {}
            if isinstance(rows, dict):
                for name, block in rows.items():
                    add_processing_block(f"$.processing.{family}.{name}", block)
    return specs
...
def validate_effect_spec(
    ...
    *,
    ...
    if raw_processor is not None:
        from .processing.catalog import canonical_processor_name

        kind = canonical_processor_name(str(raw_processor))
    else:
        kind = str(
            spec.get("kind")
            or spec.get("type")
