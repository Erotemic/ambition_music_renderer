...
def build_plugin(spec: dict[str, Any], *, base_dir: Path | None = None) -> Any:
    ...
    from ..processing.catalog import canonical_pedalboard_effect_name, pedalboard_effect_catalog

    effect = canonical_pedalboard_effect_name(
        str(spec.get("effect") or spec.get("type") or spec.get("kind") or "")
    )
    effect_meta = pedalboard_effect_catalog().get(effect, {})
    parameter_meta = effect_meta.get("parameters") or {}

    def default(name: str, fallback: Any) -> Any:
        return (parameter_meta.get(name) or {}).get("default", fallback)

    if effect == "compressor":
        return pb.Compressor(
            threshold_db=float(spec.get("threshold_db", default("threshold_db", -18.0))),
            ratio=float(spec.get("ratio", default("ratio", 3.0))),
            attack_ms=float(spec.get("attack_ms", default("attack_ms", 10.0))),
            release_ms=float(spec.get("release_ms", default("release_ms", 100.0))),
        )
    if effect == "limiter":
        return pb.Limiter(
            threshold_db=float(spec.get("threshold_db", default("threshold_db", -1.0))),
            release_ms=float(spec.get("release_ms", default("release_ms", 50.0))),
        )
    if effect == "reverb":
        return pb.Reverb(
            room_size=float(spec.get("room_size", default("room_size", 0.25))),
            damping=float(spec.get("damping", default("damping", 0.65))),
            wet_level=float(spec.get("wet_level", spec.get("wet", default("wet_level", 0.10)))),
            dry_level=float(spec.get("dry_level", default("dry_level", 1.0))),
            width=float(spec.get("width", default("width", 1.0))),
            freeze_mode=float(spec.get("freeze_mode", default("freeze_mode", 0.0))),
        )
    if effect == "chorus":
        plugin = pb.Chorus()
    ...
    if effect == "phaser":
        ...
        return plugin
    if effect == "gain":
        return pb.Gain(gain_db=float(spec.get("gain_db", spec.get("db", default("gain_db", 0.0)))))
    if effect == "distortion":
        return pb.Distortion(drive_db=float(spec.get("drive_db", default("drive_db", 12.0))))
    if effect == "clipping":
        return pb.Clipping(threshold_db=float(spec.get("threshold_db", default("threshold_db", -6.0))))
    if effect == "bitcrush":
        return pb.Bitcrush(bit_depth=float(spec.get("bit_depth", default("bit_depth", 8.0))))
    if effect == "resample":
        return pb.Resample(
            target_sample_rate=float(spec.get("target_sample_rate", spec.get("sample_rate", default("target_sample_rate", 12000.0))))
        )
    if effect == "delay":
        return pb.Delay(
            delay_seconds=float(spec.get("delay_seconds", float(spec.get("delay_ms", default("delay_ms", 90.0))) / 1000.0)),
            feedback=float(spec.get("feedback", default("feedback", 0.15))),
            mix=float(spec.get("mix", default("mix", 0.18))),
        )
    if effect == "pitch_shift":
        return pb.PitchShift(semitones=float(spec.get("semitones", default("semitones", 0.0))))
    if effect == "highpass":
        return pb.HighpassFilter(cutoff_frequency_hz=float(spec.get("cutoff_hz", spec.get("hz", default("cutoff_hz", 80.0)))))
    if effect == "lowpass":
        return pb.LowpassFilter(cutoff_frequency_hz=float(spec.get("cutoff_hz", spec.get("hz", default("cutoff_hz", 9000.0)))))
    if effect == "vst3":
        ...
        return plugin
    raise ValueError(
        f"unknown Pedalboard effect {effect!r}; checked-in catalog effects are: "
        + ", ".join(sorted(pedalboard_effect_catalog()))
    )
