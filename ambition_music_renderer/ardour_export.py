...
def _ardour_lua_misc_port_guard_lines() -> list[str]:
    """Lua lines that keep headless libardour saves out of GUI port state.

    ``Session:save_state`` also writes the process' Click/Auditioner/LTC port
    connections to the user-global ``session_port.rc``.  ``arlua`` runs on a
    Dummy backend, so allowing that side effect to escape can replace the
    user's normal metronome/hardware connections with Dummy ports.  Generated
    native-save scripts snapshot that file before loading a session and restore
    it immediately after each save.
    """

    return [
        "local user_config_dir = ARDOUR.user_config_directory(-1)",
        "local misc_port_path = user_config_dir .. '/session_port.rc'",
        "local misc_port_existed = false",
        "local misc_port_bytes = nil",
        "do",
        "  local f = io.open(misc_port_path, 'rb')",
        "  if f ~= nil then",
        "    misc_port_existed = true",
        "    misc_port_bytes = f:read('*a')",
        "    f:close()",
        "  end",
        "end",
        "local function restore_misc_port_state()",
        "  if misc_port_existed then",
        "    local f = assert(io.open(misc_port_path, 'wb'), 'cannot restore ' .. misc_port_path)",
        "    f:write(misc_port_bytes)",
        "    f:close()",
        "  else",
        "    os.remove(misc_port_path)",
        "  end",
        "end",
        "",
    ]
    ...
    text = "\n".join(
        [
            ...
            *specs,
            "}",
            "",
            *_ardour_lua_misc_port_guard_lines(),
            ...
            "local save_rc = Session:save_state('')",
            "restore_misc_port_state()",
            "assert(save_rc == 0, 'failed to save Ardour instrument realization')",
            "Session:close()",
            "",
        ]
    )
...
def _write_ardour_processing_bootstrap(
    ...
    *,
    ...
    text = "\n".join(
        [
            ...
            *_ardour_lua_misc_port_guard_lines(),
            ...
            "assert(Session ~= nil, 'failed to load Ardour session')",
            "",
            "-- new_audio_route() queues normal output auto-connect work.  Keep it disabled",
            "-- only while this native transaction owns the graph, then restore the user's",
            "-- process configuration before arlua exits.",
            "local previous_output_auto_connect = ARDOUR.config():get_output_auto_connect()",
            ...
            "local save_rc = Session:save_state('')",
            "restore_misc_port_state()",
            "ARDOUR.config():set_output_auto_connect(previous_output_auto_connect)",
            "assert(save_rc == 0, 'failed to save Ardour processing transaction')",
    ...
    # The apply pass proves the graph while the routes are live, but Python still
    # normalizes a small amount of non-route state afterwards.  Re-open the final
    # file through libardour and require the internal port graph to reconstruct.
    # This catches sessions that merely *look* connected in XML/the GUI.
    verify_path = script_path.with_name("verify_processing.lua")
    verify_text = "\n".join(
        [
            "-- Generated by ambition_music_renderer. Do not hand-edit.",
            "-- Reload the delivered session and verify the live internal audio graph.",
            "local session_dir = assert(arg[1], 'missing session directory')",
            "local snapshot = assert(arg[2], 'missing snapshot/session name')",
            "local mixspec = {",
            "  composition = { route = %s }," % _lua_string(composition_route),
            "  groups = {",
            *group_rows,
            "  },",
            "}",
            "",
            "load_session(session_dir, snapshot)",
            "assert(Session ~= nil, 'failed to reload Ardour session')",
            "",
            "local function require_route(name)",
            "  local route = Session:route_by_name(name)",
            "  assert(not route:isnil(), 'route is missing after reload: ' .. name)",
            "  return route",
            "end",
            "",
            "local function require_stereo_edge(source, destination)",
            "  for channel = 0, 1 do",
            "    local op = source:output():audio(channel)",
            "    local ip = destination:input():audio(channel)",
            "    assert(not op:isnil(), 'missing source port after reload on ' .. source:name())",
            "    assert(not ip:isnil(), 'missing destination port after reload on ' .. destination:name())",
            "    assert(op:connected_to(ip:name()), 'live edge missing after reload: ' .. op:name() .. ' -> ' .. ip:name())",
            "  end",
            "end",
            "",
            "local composition = require_route(mixspec.composition.route)",
            "for _, spec in ipairs(mixspec.groups) do",
            "  local bus = require_route(spec.route)",
            "  for _, track_name in ipairs(spec.tracks) do",
            "    require_stereo_edge(require_route(track_name), bus)",
            "  end",
            "  require_stereo_edge(bus, composition)",
            "end",
            "local master = Session:master_out()",
            "assert(not master:isnil(), 'Master route is missing after reload')",
            "require_stereo_edge(composition, master)",
            "print('Ambition Ardour live mix graph verified')",
            "Session:close()",
            "",
        ]
    )
    verify_path.write_text(verify_text, encoding="utf8")
    return script_path
...
def _strip_master_external_output_connections(root: ET.Element) -> None:
    """Keep native Master IO identity while dropping headless hardware links.

    The arlua transaction runs on Ardour's Dummy backend.  Its Master Output IO
    is a real, fully initialized native object and must remain part of the
    native route graph, but its connection to the headless backend is not
    portable.  Master is the terminal route in generated sessions, so all
    serialized connections on its output ports are external/hardware state.
    Ardour will auto-connect those ports to the user's physical outputs when
    the delivered session is opened normally.
    """

    routes_node = root.find("Routes")
    if routes_node is None:
        raise ArdourExportError("cannot sanitize Master output: Ardour session has no Routes node")
    master = next(
        (route for route in routes_node.findall("Route") if route.get("name") == "Master"),
        None,
    )
    if master is None:
        raise ArdourExportError("cannot sanitize Master output: Master route is missing")
    output = next(
        (io for io in master.findall("IO") if io.get("direction") == "Output"),
        None,
    )
    if output is None:
        raise ArdourExportError("cannot sanitize Master output: Output IO is missing")
    for port in output.findall("Port"):
        for child in list(port):
            if child.tag in {"Connection", "ExtConnection"}:
                port.remove(child)


def _preserve_native_processing_session(
    result: ArdourExportResult,
    *,
    scaffold_bytes: bytes,
) -> None:
    """Accept libardour's Routes node atomically on the pristine session.

    The previous processing merge cut across Ardour object boundaries: it mixed
    pristine MIDI Route objects with native Output IO, copied bus Route objects
    individually, and replaced the native Master's Output IO.  That can leave a
    file whose connection strings look right while the next Ardour load does not
    reconstruct the same live graph.

    Treat the complete ``<Routes>`` node produced by libardour as one native
    unit.  All non-route session state (MIDI sources/regions/playlists, timing,
    engine hints/state, config, bundles, etc.) remains byte-semantically owned by
    the already-auditioned scaffold.  Only machine-specific connections on the
    native Master's terminal output are removed before delivery.
    """
    ...
    try:
        ...
        raise ArdourExportError(f"cannot parse Ardour session after native processing save: {ex}") from ex

    native_errors = _serialized_processing_errors(result)
    if native_errors:
        raise ArdourExportError(
            "cannot preserve native processing session: " + " | ".join(native_errors)
        )

    native_routes_node = native_root.find("Routes")
    scaffold_routes_node = scaffold_root.find("Routes")
    if native_routes_node is None or scaffold_routes_node is None:
        raise ArdourExportError("cannot preserve native processing session: Routes node is missing")

    # The native Routes node will survive intact, so its MIDI playlist links must
    # still name the pristine playlists that remain outside it.
    native_routes = {str(route.get("name")): route for route in native_routes_node.findall("Route")}
    for scaffold_route in scaffold_routes_node.findall("Route"):
        playlist_id = scaffold_route.get("midi-playlist")
        if playlist_id is None:
            continue
        route_name = str(scaffold_route.get("name") or "")
        native_route = native_routes.get(route_name)
        if native_route is None:
            raise ArdourExportError(
                f"cannot preserve native processing session: MIDI route {route_name!r} is missing"
            )
        if native_route.get("midi-playlist") != playlist_id:
            raise ArdourExportError(
                f"cannot preserve native processing session: MIDI playlist identity changed on "
                f"{route_name!r} ({playlist_id!r} -> {native_route.get('midi-playlist')!r})"
            )

    route_position = list(scaffold_root).index(scaffold_routes_node)
    scaffold_root.remove(scaffold_routes_node)
    scaffold_root.insert(route_position, copy.deepcopy(native_routes_node))

    # Route/plugin objects allocated by the native pass must advance Ardour's
    # allocator watermarks even though all other root/session state stays pristine.
    for key, value in native_root.attrib.items():
        if key.endswith("-counter"):
            scaffold_root.set(key, value)

    _strip_master_external_output_connections(scaffold_root)

    if _session_timing_signature(scaffold_root) != _session_timing_signature(ET.fromstring(scaffold_bytes)):
        raise ArdourExportError(
            "internal error: native Routes merge changed MIDI/session timing"
        )
...
def apply_ardour_processing(
    ...
    *,
    ...
    if not errors:
        _restore_ardour_midi_sources(result, midi_snapshot)
        try:
            _preserve_native_processing_session(result, scaffold_bytes=scaffold_bytes)
        except ArdourExportError as ex:
            errors = [str(ex)]
        if not errors:
            post = _serialized_processing_errors(result)
            if not post:
                verify_script = result.processing_bootstrap.with_name("verify_processing.lua")
                if not verify_script.is_file():
                    errors = [f"processing live-graph verifier is missing: {verify_script}"]
                else:
                    verify_command = [
                        str(exe),
                        str(verify_script),
                    ...
                    _restore_ardour_midi_sources(result, midi_snapshot)
                    verify_details = "\n".join(
                        part for part in (verified.stdout, verified.stderr) if part.strip()
                    )
                    if "Ambition Ardour live mix graph verified" in verified.stdout:
                        _update_processing_status(result, status="applied", ardour_lua=exe)
                        return completed
                    errors = [
                        "final Ardour session did not reconstruct the native live mix graph on reload"
                    ]
                    if verify_details:
                        details = (details + "\n" + verify_details).strip()
            else:
                errors = post
    ...
    *,
    ...
    manifest_data = {
        ...
        "processing_transport": {
            ...
            "session_merge": "libardour_native_routes_node_onto_pristine_session",
            "bootstrap": (
            ...
            "verification_bootstrap": (
                str(processing_bootstrap.with_name("verify_processing.lua").relative_to(destination))
                if processing_bootstrap is not None
                else None
            ),
            "live_graph_verification": "arlua_reload_port_connected_to",
