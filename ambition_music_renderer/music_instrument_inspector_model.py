...
def _effect_templates_from_catalog() -> dict[str, dict[str, Any]]:
    from .processing.catalog import inspector_templates

    out: dict[str, dict[str, Any]] = {}
    for name, raw in inspector_templates().items():
        row = dict(raw)
        # Preserve the historical inspector/export spelling while sourcing the
        # template vocabulary from the checked-in processing catalog.
        if "processor" in row and "kind" not in row:
            row["kind"] = row.pop("processor")
        out[str(name)] = row
    return out


EFFECT_TEMPLATES: dict[str, dict[str, Any]] = _effect_templates_from_catalog()
