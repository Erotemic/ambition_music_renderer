...
def test_canonical_processing_changes_render_fingerprint(tmp_path):
    renderer_root = _fake_renderer_root(tmp_path)
    base = _score()
    base["processing"] = {
        "master": {"chain": [{"processor": "gain", "gain_db": -1.0}]},
        "adaptive_sections": {"mode": "composition_master"},
    }
    changed = deepcopy(base)
    changed["processing"]["master"]["chain"][0]["gain_db"] = -2.0
    before = _fingerprint(tmp_path, base, package_root=renderer_root)
    after = _fingerprint(tmp_path, changed, package_root=renderer_root)
    assert before.fingerprint != after.fingerprint
    assert before.payload["audio_settings"]["mastering_policy"]["mode"] == "composition_master"


def test_canonical_processing_tracks_vst3_bundle_identity(tmp_path):
    renderer_root = _fake_renderer_root(tmp_path)
    plugin = tmp_path / "CanonicalAmp.vst3"
    binary = plugin / "Contents" / "x86_64-linux" / "CanonicalAmp.so"
    binary.parent.mkdir(parents=True)
    binary.write_bytes(b"canonical-plugin-v1")
    ...
    before = _fingerprint(tmp_path, spec, package_root=renderer_root)
    binary.write_bytes(b"canonical-plugin-v2-with-different-size")
    after = _fingerprint(tmp_path, spec, package_root=renderer_root)
    assert before.fingerprint != after.fingerprint
