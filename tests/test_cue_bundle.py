...
def test_bundle_without_publish_does_not_require_publish_root(monkeypatch):
    from ambition_music_renderer.render import bundle as bundle_mod

    def forbidden_publish_root():
        raise AssertionError("non-publishing bundles must not resolve publish authority")

    monkeypatch.setattr(bundle_mod, "default_publish_dest_root", forbidden_publish_root)
    assert bundle_mod._resolve_bundle_dest_root(None, publish=False) is None


def test_bundle_publish_root_resolution_is_lazy(monkeypatch, tmp_path):
    from ambition_music_renderer.render import bundle as bundle_mod

    configured = tmp_path / "game-assets"
    monkeypatch.setattr(bundle_mod, "default_publish_dest_root", lambda: configured)
    assert bundle_mod._resolve_bundle_dest_root(None, publish=True) == configured
    explicit = tmp_path / "explicit"
    assert bundle_mod._resolve_bundle_dest_root(explicit, publish=False) == explicit
