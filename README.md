...
## Architecture campaign recovery

For cross-cutting renderer work, start with `docs/architecture_roadmap.md`. The
core compatibility promise is that committed MusicIR v1/v2 scores remain
regenerable while v3 is the authoring model for new music. Source-only agents
can inspect the expected instrument vocabulary in
`ambition_music_renderer/data/instrument_catalog.yaml`, the captured maintainer
workstation in `ambition_music_renderer/data/audio_environment_snapshot.json`,
the public v3 generator surface in
`ambition_music_renderer/data/generator_catalog.json`, and the canonical
render-time processing vocabulary in
`ambition_music_renderer/data/processing_catalog.json`, the technique vocabulary
in `ambition_music_renderer/data/technique_catalog.json`, and the derived
instrument authoring index without access to the user's `/data/audio-tools`
tree. For a dependency-free overview use `python -S dev/music_authoring_reference.py summary`.



## New MusicIR v3 composition

V3 is the default for new scores. Existing v1/v2 sources remain supported
regeneration inputs and should not be rewritten merely to adopt the new schema.

Create a dependency-light starter with the installed package:

```bash
python -m ambition_music_renderer cue new "My New Cue"
```

or from a source checkout with no third-party dependencies:

```bash
python -S dev/new_musicir_v3.py "My New Cue"
```

Useful authoring/review surfaces are:

```bash
python -m ambition_music_renderer cue validate <score>
python -m ambition_music_renderer cue graph <score>
python -m ambition_music_renderer cue expand <score> --clip <clip-id>
...
It compiles every committed v1/v2/implicit-v1 score through the canonical
`CompiledScore` boundary. For broad compiler refactors, compare ephemeral pre/post
reports rather than checking named-song fingerprints into the test suite.

## Processing and mastering

New v3 scores can author a canonical static render chain under `processing:`.
The checked-in `data/processing_catalog.json` is consumed by validation, the
renderer, plugin dispatch, inspector templates, and dependency tracking. Inspect
it with:
...
A source-only agent can use `python -S dev/read_processing_catalog.py list`.
Existing flat `postprocess` fields remain supported for v1/v2 regeneration. New
v3 adaptive music uses one composition master by default; adaptive full mixes
are slices of that mastered timeline. See `docs/processing_architecture.md`.
...
V2/v3 MIDI previews are written from the exact score clock, including meter and
...
All MusicIR authoring frontends compile through
`ambition_music_renderer.musicir.compile.compile_score()` into a shared
`CompiledScore`. V1 remains the established procedural language, v2 remains the
exact-score language, and v3 introduces a unified authoring graph where exact
clips and procedural generator clips can coexist. Renderer, audit, timeline,
inspector, and export code should consume `CompiledScore` rather than reinterpret
raw YAML or private `PrettyMIDI` metadata. The historical
`render.score_layers.build_score()` tuple API remains a migration compatibility
facade.
...
python -m ambition_music_renderer cue validate <cue_id> --json
```

V3 validation reports source-oriented diagnostics with YAML paths, line/column
coordinates, stable error codes, and typo/repair hints before compilation where
possible. Inspect the derivation without rendering audio with:

```bash
python -m ambition_music_renderer cue graph <cue_id>
python -m ambition_music_renderer cue expand <cue_id> --clip <clip_id>
python -m ambition_music_renderer cue trace_event <cue_id> <event_id>
```

These inspection commands reuse `NormalizedScoreGraph`, `CompiledScore`, exact
harmony timing, and `InstrumentResolutionPlan`; they are not alternate
compilers/resolvers. See `docs/musicir_diagnostics.md`.

Validation also reports a deterministic compiled-score fingerprint so refactors can
...
For new schema work, see `docs/musicir_v3.md`. V3 is the direction for a single
authoring model that can be concise when musical intent is generative and exact
when literal notation is clearer. It adds stable score/part/voice/clip identity
for future DAW reconciliation while continuing to compile to the same
`CompiledScore` renderer contract. Existing v1/v2 cues remain supported and are
not candidates for mechanical conversion without an authoring benefit and
equivalence evidence.

The fields below describe the established v1 procedural vocabulary, still used
by most current cues. MusicIR is intentionally declarative. Common top-level
fields include:
